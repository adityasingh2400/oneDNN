...
This is different from how a scale tensor is set for primitive attributes where
the scale tensor has the same rank as the source tensor, with an extent of `1`
for every unmasked dimension. For example, the scale tensor has shape `[B, H, 1,
1]` for `mask=3` in the primitive API, while the proposed Graph API scale tensor
has shape `[B, H]`.
...
When `mask` is present, it defines the non-group scale granularity and takes
precedence over `qtype` and `axis`. The initial proposal is to reject a graph
that specifies both `mask` and a non-default `qtype` or `axis`, rather than
silently resolving conflicting descriptions. A later API revision may deprecate
`qtype` and `axis` after the adoption of `mask` is sufficient.
